...
class Demo extends Component {
    ...
    dragEnd = () => {
        const { isDragEnter, currentTarget, addFlowEditor, replacedIndex, insertIndex } = this.props;
        ...
        if (currentTarget && utils.isNumber(insertIndex)) {
            ...
            addFlowEditor(Object.assign(newFlowEditor, {
                edit: false,
                key: ++key
            }));
        }
    ...
    flowDragEnd = () => {
        ...
        } else if (utils.isNumber(flowDragIndex) && utils.isNumber(insertIndex)) {
                                ...
                                if (item.type === "dimension") {
                                    return (
                                        <Fragment key={item.key}>
                                            <div
                                                ...
                                                onDragEnter={() => this.setInsertIndex(index)}
                                                onDragLeave={() => this.setInsertIndex(undefined)}
                                            >
                                            </div>
                                            <div
                                                className={`dimension ${item.edit ? "nopadding" : ""}`}
                                                draggable
                                                onDoubleClick={() => this.onDoubleClick(item.key)}
                                                onDragOver={(e) => this.dragOver(e)} //控制鼠标样式
                                                onDragEnter={() => this.setReplacedIndex(index)}
                                                onDragLeave={() => this.setReplacedIndex(undefined)}

                                                onDragStart={() => this.props.setFlowDragIndex(index)}
                                                onDragEnd={this.flowDragEnd}
                                            >
                                                {
                                                    item.edit ? <input value={item.name} /> : item.name
                                                }
                                            </div>
                                        </Fragment>
                                    )
                                    ...
                                    return (
                                        <Fragment key={item.key}>
                                            ...
                                            <div
                                                className={`formula ${item.edit ? "nopadding" : ""}`}
                                                draggable
                                                onDoubleClick={() => this.onDoubleClick(item.key)}
                                                onDragOver={(e) => this.dragOver(e)} //控制鼠标样式
                                                onDragEnter={() => this.setReplacedIndex(index)}
                                            ...
                                            >
                                                {
                                                    item.edit ? <input value={item.text} /> : item.text
                                                }
                                            </div>
                                        </Fragment>
                                    )
