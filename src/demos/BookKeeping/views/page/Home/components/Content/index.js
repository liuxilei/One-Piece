    ...
    setMode = (mode) => {
        this.setState({
            ...
            if (mode === "图表模式") {
                ...
                this.expenditureChart = echarts.init(this.expenditureRef.current);
                let food = {
                    name: "餐饮",
                    value: 0,
                };
                ...
                let data = this.props.bookKeeping.get(this.props.currentDate);
                if (data) {
                    data.map(item => {
                        ...
                        if (item.get("wayType") === dailyNecessities.name) {
                            dailyNecessities.value += parseFloat(item.get("money"));
                        }
                        ...
                        if (item.get("wayType") === financialManagement.name) {
                            ...
                            data ? data.map(item => {
                                return (
                                    <BookKeepingRecordItem key={item.get("id")}>
                                        <div className="item-icon"><i className="iconfont" dangerouslySetInnerHTML={{ __html: nameToIcon[item.get("wayType")] }}></i></div>
                                        <div className="item-text">{item.get("title")}</div>
                                        <div className="item-money">{`${item.way === "expenditure" ? "-" : ""}${item.get("money")}元`}</div>
                                        <div className="item-time">{currentDate}</div>
                                        <div className="item-operate">
                                            <i className="iconfont" onClick={() => this.editStart(item)}>&#xe628;</i>
                                            <i className="iconfont" onClick={() => deleteRecordItem(item.get("id"))}>&#xe747;</i>
                                        </div>
                                    </BookKeepingRecordItem>
                                )
                            }) :
