...
class Edit extends PureComponent {
    ...
    submit = () => {
        ...
        let id;
        let isEdit = false;
        if (this.props.currentEditRecord && this.props.currentEditRecord.get("id")) {
    ...
    render() {
        ...
        return (
            <EditWrapper>
                <Tabs>
                    <div className={`${way === "expenditure" ? "select" : ""}`} onClick={() => this.currySetState("way", "expenditure")}>支出</div>
                    <div className={`${way === "income" ? "select" : ""}`} onClick={() => this.currySetState("way", "income")}>收入</div>
                </Tabs>
                <TypeWays>
                    {
                        typeList[way].map(item => {
                            return (
                                <TypeWaysItem key={item.name} onClick={() => this.currySetState("wayType", item.name)}>
                                    <div className={`icon ${item.name === wayType ? "select" : ""}`}><i className="iconfont" dangerouslySetInnerHTML={{ __html: item.unicode }}></i></div>
                ...
                <FormLayout>
                    ...
                    <div className="input">
                        <input
                            ...
                            className="normal"
                            value={title}
                            onChange={e => this.currySetState("title", e.target.value)}
                        />
                    </div>
                </FormLayout>
                <FormLayout>
                    ...
                    <div className="input">
                        <input
                            ...
                            value={money}
                            className="normal"
                            onChange={e => this.currySetState("money", e.target.value)}
                        />
                    </div>
                </FormLayout>
                <FormLayout>
                    ...
                    <div className="input">
                        <DatePicker
                            ...
                            format={dateFormat}
                            className="date"
                            onChange={(date, dateString) => this.currySetState("time", dateString)}
                        />
                    </div>
                </FormLayout>
                <ButtonWrapper>
                    <OperateButton className="primary" onClick={this.submit}>提交</OperateButton>
                    <Link to="/BookKeeping">
